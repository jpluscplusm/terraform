...
func (c *TestCommand) ExecuteTestSuite(suite *moduletest.Suite, config *configs.Config, view views.Test) {
	...
	schemas, schemaDiags := ctx.Schemas(config, nil)
	...
	for _, name := range files {
		...
		c.ExecuteTestFile(ctx, file, schemas, config, view)

		suite.Status = suite.Status.Merge(file.Status)
	}
}

func (c *TestCommand) ExecuteTestFile(ctx *terraform.Context, file *moduletest.File, schemas *terraform.Schemas, config *configs.Config, view views.Test) {
	var diags tfdiags.Diagnostics
